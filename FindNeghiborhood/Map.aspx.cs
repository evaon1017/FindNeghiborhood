    ...
    public partial class Map : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.UI_repCities.ItemDataBound += (a, b) =>
            {
                ...
                var minX = groupedRows.Min(r => r.Field<int>("X"));
                var minY = groupedRows.Min(r => r.Field<int>("Y"));
                var maxX = groupedRows.Max(r => r.Field<int>("X"));
                var maxY = groupedRows.Max(r => r.Field<int>("Y"));

                ((HtmlGenericControl)b.Item.FindControl("UI_mapName")).InnerText = $"{groupedRows.Key.Item1} - {groupedRows.Key.Item2}";

                var dataSet = groupedRows.ToDictionary(r => Tuple.Create(r.Field<int>("X"), r.Field<int>("Y")), r => new
                {
                ...
                for (int y = minY; y <= maxY; y++)
                {
                    ...
                    for (int x = minX; x <= maxX; x++)
                    {
                        ...
                        if (dataSet.ContainsKey(key))
                        {
                            ...
                            {
                                Text = $"Token id {unit.Token}",
                            });

                            cell.Controls.Add(new Literal
                            {
                                Text = $"<br />{unit.Cord}<br />",
                            });
